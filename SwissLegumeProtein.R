...
SwissRuminantProteinConsumption <- SwissRuminantProduction * 0.214
SwissMonogastricProteinConsumption <- SwissMonogastricProduction * 0.216
SwissMeatProteinConsumption <- SwissMonogastricProteinConsumption+SwissRuminantProteinConsumption
...
MeatProteinConsumption-SwissMeatProteinConsumption

NettoImportedMeatProtein <- MeatProteinConsumption-SwissMeatProteinConsumption
...
get_areas_and_protein_production <- function(areas_raw, MeatProteinConsumption, reduce_lay_maize_prop, LegumeArea, Temp_grassland_area, lay_yield, SiloMais, prot_tha ){
  ...
  areas$Replacing_meat_protein_single <- areas$Protein_production_single/MeatProteinConsumption*100
  ...
  areas$Replacing_meat_protein <- areas$Protein_production_combined/MeatProteinConsumption*100
...
p <- TableArea_Small[,c(3,1,2,5,4,6:13,16)]

units <- data.frame(" "," "," ","(t/ha)","(ha)","(ha)","(t)","(\\%)","(ha)","(ha)","(ha)","(ha)","(t)","(\\%)")
names(units) <- names(p)
p <- rbind(units,p)
p
require(xtable)
...
####################################
names(TableArea_Small)
p <- TableArea_Small[,c(3,1,2,5,4,6:8,15,9,19,10:11, 14:15,12:13,16)]
p$`Production on perm grasslands combined` <- format(p$`Production on perm grasslands combined`,big.mark = ",")


p <- p[,c(1:5,10:18)]
units <- data.frame(" "," "," ","(t/ha)","(ha)","(ha)","(ha)","(ha)","(ha)","(t)","(t)","(ha)","(t)","(\\%)")
names(units) <- names(p)
p <- rbind(units,p)
p
require(xtable)
...
##########
FeedMilkProtein <- 370000*0.033
# 120 086 TS  https://www.sbv-usp.ch/fileadmin/user_upload/04_SES2021_Versorgungsbilanzen.pdf  
TotalMilkProtein <- 3400000 *0.033 # 3.4 Mio t Verkehrsmilk https://www.sbv-usp.ch/fileadmin/user_upload/MISTA2020_def_online.pdf

Protein_food_total <- MeatProteinConsumption+TotalMilkProtein

####
Area_Perm_grassland_for_legumes <- abs(unique(areas_all_yield$Area_Perm_grassland_for_legumes))

get_balance <- function(Total_feed_raw,Temp_grassland_area,lay_yield, reduce_lay_maize_prop, Area_Perm_grassland_for_legumes, PermanentPerm_grasslands, Protein_food_total, Protein_legume_total,Fodder_Mais_half,SwissRuminantProteinConsumption,SwissMonogastricProteinConsumption,NettoImportedMeatProtein){
  ...
  # ratio_herb_mono_raw_2017 <-(7251404-601150)/(1255657 -1016709)
  ...
  1255657*0.067
  Raw_monogastrics_total <- Conc_monogastrics_total/0.81*0.067 # https://www.sbv-usp.ch/fileadmin/sbvuspch/00_Bilder/04_Medien/Agristat_aktuell/2019/Aktuell_AGRISTAT_2019-04.pdf
  Raw_ruminants_total <- Total_feed_raw-Raw_monogastrics_total
  ...
  Total_herbivor_feed <- Raw_ruminants_total+Conc_ruminats_total+Total_herbivor_feed_other
  Total_Monogastrics_feed <- Raw_monogastrics_total+Conc_monogastrics_total+Total_Monogastrics_feed_other
  
  ratio_herb_mono_raw_2020 <- Raw_ruminants_total/Raw_monogastrics_total
  
  Total_feed_conc-Conc_ruminats_total-Conc_monogastrics_total
  Total_feed_raw-Raw_ruminants_total-Raw_monogastrics_total
  Total_feed_all-Total_herbivor_feed-Total_Monogastrics_feed
  
  PropOfHerbIn_raw <- Raw_ruminants_total/Total_feed_raw
  ...
  Fodder_pasture <- Total_feed_raw - Fodder_Temp_grassland #####Raw_ruminants_total-Fodder_Temp_grassland
  ...
  Fodder_Perm_grassland_reduced/Raw_ruminants_total
  
  Reduced_Herbivors_TS_Proportion <- (Fodder_Temp_grassland+Reduction_Perm_grassland_TS)/Raw_ruminants_total## reduction in TS
  ...
  Table_feed1 <- data.frame(Scenario=c("Scenario II"),Animal=c("Ruminants"), Feed=c("Cereal","Concentrated","Side Products","Conc others","Others","Raw feed"), Total_feed= c(Total_herbivor_feed), Proportion=c(0.031, 0.083, 0.043,NA,0.043,0.874))
  ...
  Table_feed1$Feed_reduction[Table_feed1$Feed=="Raw feed"] <-  Fodder_Temp_grassland+Reduction_Perm_grassland_TS
  ...
  Table_feed_Monogastrics <- data.frame(Animal=c("Monogastrics"), Feed=c("Cereal","Concentrated","Side Products","Conc others","Others","Raw feed"), Total_feed= c(Total_Monogastrics_feed), Proportion=c(50.2/100, propConc , 25.6/100,NA ,12.3/100, 6.7/100 )) #81/100
  ...
  # Protein_food_total <- SwissMeatProteinConsumption+TotalMilkProtein+NettoImportedMeatProtein
  
  # 
  # 
  # ## 2
  # Protein_legume_food <- Protein_food_total-r_monogastrics_feedback* SwissMonogastricProteinConsumption-(TotalMilkProtein+SwissRuminantProteinConsumption)*r_ruminants-NettoImportedMeatProtein
  # 
  # 
  # ### 3
  ...
  # 
  # ### 4
  # Conc_monogastrics_new <- Conc_monogastrics_netto+Conc_legume_feed+Conc_ruminats_total*(1-r_ruminants) #Monogastric_conc_prox
  # Conc_monogastrics_new
  # 
  # ### 5
  # r_ruminants <- (Raw_ruminants_total+Raw_monogastrics_total*(1-r_monogastrics_feedback)-Fodder_Temp_grasslandMais-Reduction_Perm_grassland_TS)/Raw_ruminants_total #r_ruminants
  # 
  # ### 6
  # r_monogastrics <- Conc_monogastrics_new/Conc_monogastrics_total## 
  # #r_monogastrics_feedback
  # 
  # ### 6.5
  # #Total_feed_raw * r_ruminants +Total_feed_raw *r_monogastrics  <- Raw_ruminants_total-Fodder_Temp_grasslandMais-Reduction_Perm_grassland_TS
  # Total_feed_raw * r_ruminants +Total_feed_raw *r_monogastrics  <- Total_feed_raw-Fodder_Temp_grasslandMais-Reduction_Perm_grassland_TS
  # r_monogastrics <- (Total_feed_raw-Fodder_Temp_grasslandMais-Reduction_Perm_grassland_TS)/Total_feed_raw - (r_ruminants) 
  # 
  # 
  # ### 7 (6 in 5)
  # r_ruminants <-(Raw_ruminants_total+Raw_monogastrics_total*(1-(Conc_monogastrics_new/Conc_monogastrics_total))-Fodder_Temp_grasslandMais-Reduction_Perm_grassland_TS)/Raw_ruminants_total #r_ruminants
  # 
  # 
  # ### 8 (3 in 4)
  # Conc_monogastrics_new <- Conc_monogastrics_netto+((Protein_legume_total -Protein_legume_food) * Protein2OilCake)+Conc_ruminats_total*(1-r_ruminants) #Monogastric_conc_prox
  # 
  # ## 9 (6.5 in 2)
  # Protein_legume_food <- Protein_food_total-( (Total_feed_raw-Fodder_Temp_grasslandMais-Reduction_Perm_grassland_TS)/Total_feed_raw - (r_ruminants)  )* SwissMonogastricProteinConsumption-(TotalMilkProtein+SwissRuminantProteinConsumption)*r_ruminants-NettoImportedMeatProtein
  # 
  # ## 10 (9 in 8)
  # Conc_monogastrics_new <- Conc_monogastrics_netto+((Protein_legume_total - (Protein_food_total-( (Total_feed_raw-Fodder_Temp_grasslandMais-Reduction_Perm_grassland_TS)/Total_feed_raw - (r_ruminants)  )* SwissMonogastricProteinConsumption-(TotalMilkProtein+SwissRuminantProteinConsumption)*r_ruminants-NettoImportedMeatProtein) ) * Protein2OilCake)+Conc_ruminats_total*(1-r_ruminants) #Monogastric_conc_prox
  # 
  # # 11 (10 in 7)
  # r_ruminants <-(Raw_ruminants_total+Raw_monogastrics_total*(1-(  (Conc_monogastrics_netto+((Protein_legume_total - (Protein_food_total-( (Total_feed_raw-Fodder_Temp_grasslandMais-Reduction_Perm_grassland_TS)/Total_feed_raw - (r_ruminants)  )* SwissMonogastricProteinConsumption-(TotalMilkProtein+SwissRuminantProteinConsumption)*r_ruminants-NettoImportedMeatProtein) ) * Protein2OilCake)+Conc_ruminats_total*(1-r_ruminants) )   /Conc_monogastrics_total))-Fodder_Temp_grasslandMais-Reduction_Perm_grassland_TS)/Raw_ruminants_total #r_ruminants
  # 
  # 
  ...
  f <- function(r_ruminants) {
    r_ruminants <-(Raw_ruminants_total+Raw_monogastrics_total*(1-(  (Conc_monogastrics_netto+((Protein_legume_total - (Protein_food_total-( (Raw_ruminants_total-Fodder_Temp_grasslandMais-Reduction_Perm_grassland_TS)/Total_feed_raw - (r_ruminants)  )* SwissMonogastricProteinConsumption-(TotalMilkProtein+SwissRuminantProteinConsumption)*r_ruminants-NettoImportedMeatProtein) ) * Protein2OilCake)+Conc_ruminats_total*(1-r_ruminants) )   /Conc_monogastrics_total))-Fodder_Temp_grasslandMais-Reduction_Perm_grassland_TS)/Raw_ruminants_total #r_ruminants
  }
  p <- optimize(f, interval = c(0.5,1), maximum = T)
  
  r_ruminants <- p$objective
  r_ruminants
  ##
  
  Conc_monogastrics_new <- Conc_monogastrics_netto+((Protein_legume_total - (Protein_food_total-( (Raw_ruminants_total-Fodder_Temp_grasslandMais-Reduction_Perm_grassland_TS)/Total_feed_raw - (r_ruminants)  )* SwissMonogastricProteinConsumption-(TotalMilkProtein+SwissRuminantProteinConsumption)*r_ruminants)-NettoImportedMeatProtein ) * Protein2OilCake)+Conc_ruminats_total*(1-r_ruminants) #Monogastric_conc_prox
  
  
  r_monogastrics_feedback <- Conc_monogastrics_new/Conc_monogastrics_total
  ...
  ####
  Protein_legume_food
  FractionForPlantProtein <- Protein_legume_food/SwissMeatProteinConsumption
  FractionForPlantProtein
  
  
  SwissRuminantProteinConsumption_new <- SwissRuminantProteinConsumption*(r_ruminants)
  SwissMonogastricProteinConsumption_new <- SwissMonogastricProteinConsumption*(r_monogastrics_feedback)
  SwissRuminantProteinConsumption_new+SwissMonogastricProteinConsumption_new-SwissRuminantProteinConsumption-SwissMonogastricProteinConsumption
  
  MilkproteinToSubstitute_feedback <- TotalMilkProtein*(1-r_ruminants)
  MilkproteinToSubstitute_feedback
  ##########
  
  
  protein_df <- NULL
  protein_df <- data.frame(Source=c("Milk"),Animal="Ruminant",Scenario="Milk",Protein=TotalMilkProtein-MilkproteinToSubstitute_feedback)
  protein_df <- rbind(protein_df, data.frame(Source=c("Milk"),Animal="Legume",Scenario="Plant",Protein=MilkproteinToSubstitute_feedback))
  # protein_df <- rbind(protein_df, data.frame(Source=c("Plant"),Scenario="Used",Protein=x))
  protein_df <- rbind(protein_df, data.frame(Source=c("Meat"),Animal="Ruminant",Scenario="Meat",Protein=SwissRuminantProteinConsumption_new))
  protein_df <- rbind(protein_df, data.frame(Source=c("Meat"),Animal="Monogastrics",Scenario="Meat",Protein=SwissMonogastricProteinConsumption_new))
  protein_df <- rbind(protein_df, data.frame(Source=c("Meat"),Animal="Legume",Scenario="Plant",Protein=SwissMeatProteinConsumption-SwissMonogastricProteinConsumption_new-SwissRuminantProteinConsumption_new))
  ...
  Table_feed_Monogastrics$Legume_feed_CH <-  Conc_legume_feed
  Table_feed_Monogastrics$Legume_feed_CH[!(Table_feed_Monogastrics$Feed%in%c("Concentrated","Side Products"))] <- 0
  
  Table_feed_Monogastrics$ReducedTo_Proportion <- r_monogastrics_feedback
  FeedData <- rbind.fill(Table_feed_Ruminants,Table_feed_Monogastrics)
  
  FeedDataDetail <- FeedData[!FeedData$Feed%in%c("Concentrated","Raw feed"),]
  ...
  FeedDataDetail$Feed[FeedDataDetail$Feed_item=="Others"] <- "Raw feed"
  
  TableFeed_Small <- subset(FeedDataDetail, Scenario=="Scenario II"&Feed_item!="Others")#TableFeed[order(TableFeed$Scenario),]
  TableFeed_Small2 <- setDT(TableFeed_Small)[,list(Feed_baseline=sum(Quantity),Import=sum(Import),Legume_feed_CH=sum(Legume_feed_CH,na.rm=T),Reduction_in_conc_feed=sum(Reduction_in_conc_feed,na.rm = T),Feed_reduced=sum(Feed_reduced)),by=.(Scenario,Animal,Feed,Feed_item,ReducedTo_Proportion)]
  ...
  ########
  ######
  Reduced_RawFeed_TS <- data.frame(Fodder_maize=-Fodder_Mais_half, Temp_grassland=-Fodder_Temp_grassland_half, Perm_grassland=-Reduction_Perm_grassland_TS) #, Add_Raw_feed_from_monogastric= ReducedMonogastricRawFeed
  Total=sum(Reduced_RawFeed_TS)
  Others=0
  Reduced_RawFeed_TS <- rbind(t(Reduced_RawFeed_TS), Others, Total)
  Reduced_RawFeed_TS <- as.data.frame(Reduced_RawFeed_TS)
  names(Reduced_RawFeed_TS)[1] <- "Reduction_in_raw_feed"
  Reduced_RawFeed_TS$Reduction_in_raw_feed_area[rownames(Reduced_RawFeed_TS)=="Fodder_maize"] <- -SiloMais*reduce_lay_maize_prop
  Reduced_RawFeed_TS$Reduction_in_raw_feed_area[rownames(Reduced_RawFeed_TS)=="Temp_grassland"] <- -Temp_grassland_area*reduce_lay_maize_prop
  Reduced_RawFeed_TS$Reduction_in_raw_feed_area[rownames(Reduced_RawFeed_TS)=="Perm_grassland"] <- -Area_Perm_grassland_for_legumes
  Reduced_RawFeed_TS$Reduction_in_raw_feed_area[rownames(Reduced_RawFeed_TS)=="Total"] <-sum(Reduced_RawFeed_TS$Reduction_in_raw_feed_area, na.rm = T)
  
  Reduced_RawFeed_TS$Feed_baseline[rownames(Reduced_RawFeed_TS)=="Total"] <- Total_feed_raw#Raw_ruminants_total
  Reduced_RawFeed_TS$Feed_baseline[rownames(Reduced_RawFeed_TS)=="Fodder_maize"] <- Production_Mais_Feed#*(PropOfHerbIn_raw)
  Reduced_RawFeed_TS$Feed_baseline[rownames(Reduced_RawFeed_TS)=="Temp_grassland"] <- Fodder_Temp_grassland#*(PropOfHerbIn_raw)
  Perm_grasslands_Temp_grasslands_TS <- Total_feed_raw-970313## 4906452 #mehrjahrigerFutterbau TS
  Reduced_RawFeed_TS$Feed_baseline[rownames(Reduced_RawFeed_TS)=="Perm_grassland"] <- (Perm_grasslands_Temp_grasslands_TS-Fodder_Temp_grassland)#*(PropOfHerbIn_raw) # https://www.sbv-usp.ch/fileadmin/user_upload/04_SES2021_Versorgungsbilanzen.pdf
  Reduced_RawFeed_TS$Feed_baseline[rownames(Reduced_RawFeed_TS)=="Others"] <- Total_feed_raw-sum(Reduced_RawFeed_TS$Feed_baseline[rownames(Reduced_RawFeed_TS)!="Total"],na.rm=T)
  sum(Reduced_RawFeed_TS$Feed_baseline[rownames(Reduced_RawFeed_TS)!="Total"],na.rm=T)-Total_feed_raw
  
  Reduced_RawFeed_TS$Proportion[rownames(Reduced_RawFeed_TS)=="Total"] <- Reduced_RawFeed_TS$Reduction_in_raw_feed[rownames(Reduced_RawFeed_TS)=="Total"] /Reduced_RawFeed_TS$Feed_baseline[rownames(Reduced_RawFeed_TS)=="Total"] 
  Reduced_RawFeed_TS$Proportion <- round(Reduced_RawFeed_TS$Proportion, digits = 3)
  Reduced_RawFeed_TS$Reduction_in_raw_feed <- round(Reduced_RawFeed_TS$Reduction_in_raw_feed, digits = 1)
  Reduced_RawFeed_TS$Animal <- "Ruminants"
  Reduced_RawFeed_TS$Feed_item <- rownames(Reduced_RawFeed_TS)
  Reduced_RawFeed_TS$Feed <- "Raw feed"
  
  ###
  Reduced_Herbivors_TS <- Reduced_RawFeed_TS[,c("Animal","Feed","Feed_item","Reduction_in_raw_feed","Reduction_in_raw_feed_area","Feed_baseline")]
  Reduced_Herbivors_TS$Feed_baseline <- Reduced_Herbivors_TS$Feed_baseline*PropOfHerbIn_raw
  
  Reduced_Mono_TS <- Reduced_RawFeed_TS[,c("Feed","Feed_item","Reduction_in_raw_feed","Reduction_in_raw_feed_area","Feed_baseline")]
  Reduced_Mono_TS$Animal <- "Monogastrics"
  Reduced_Mono_TS$Feed_baseline <- Reduced_Mono_TS$Feed_baseline*(1-PropOfHerbIn_raw)
  Reduced_Mono_TS$Reduction_in_raw_feed_area <- 0#Reduced_Mono_TS$Reduction_in_raw_feed_area*(1-PropOfHerbIn_raw)
  Reduced_Mono_TS$Reduction_in_raw_feed <- -Reduced_Mono_TS$Feed_baseline*(1-r_monogastrics_feedback)
  
  
  require(plyr)
  Reduced_TS <- rbind.fill(Reduced_Herbivors_TS,Reduced_Mono_TS)
  Reduced_TS <- Reduced_TS[Reduced_TS$Feed_item!="Total",c("Animal","Feed","Feed_item","Feed_baseline","Reduction_in_raw_feed_area","Reduction_in_raw_feed")]
  Reduced_TS$Feed_baseline[is.na(Reduced_TS$Feed_baseline)] <- 0
  Reduced_TS$Reduction_in_raw_feed[is.na(Reduced_TS$Reduction_in_raw_feed)] <- 0
  Reduced_TS$Reduction_in_raw_feed_monogastrics[Reduced_TS$Animal=="Ruminants"] <- -Reduced_TS$Reduction_in_raw_feed[Reduced_TS$Animal=="Monogastrics"] 
  Reduced_TS$Reduction_in_raw_feed_monogastrics[Reduced_TS$Animal=="Monogastrics"] <- 0
  
  Reduced_TS$Feed_reduced <-Reduced_TS$Feed_baseline+Reduced_TS$Reduction_in_raw_feed+Reduced_TS$Reduction_in_raw_feed_monogastrics
  ...
  Reduced_TS$Reduction_in_raw_feed_area[is.na(Reduced_TS$Reduction_in_raw_feed_area)] <- 0
  table_reduced_raw_feed <-  Reduced_TS
  
  table_reduced_raw_feed$Proportion <- table_reduced_raw_feed$Feed_reduced/table_reduced_raw_feed$Feed_baseline
  
  return(list(table_reduced_raw_feed, table_reduced_concentrated_feed,protein_df))
}

####

Total_feed_raw_variation <- seq.int(5667296, 5799925,length.out=10)

balance <- lapply(Protein_legume_total, function(i_prot_legume) lapply(Total_feed_raw_variation, function(i_feed) get_balance(Total_feed_raw=i_feed, Temp_grassland_area,lay_yield, reduce_lay_maize_prop, Area_Perm_grassland_for_legumes, PermanentPerm_grasslands, Protein_food_total, i_prot_legume,Fodder_Mais_half,SwissRuminantProteinConsumption,SwissMonogastricProteinConsumption,NettoImportedMeatProtein)))
...
get_results <- function(balance, n_results){
  ...
  for (iteration_outer in 1:iterations){
    for (iteration2 in 1:iterations){
      ...
      p$Total_feed_raw <- Total_feed_raw_variation[iteration2]
...
table_reduced_raw_feed <- get_results(balance, 1)

p <- melt.data.table(table_reduced_raw_feed, measure.vars = c("Proportion"))
# p <- subset(p, Animal%in%c("Total Ruminants","Total Monogastrics"))
ggplot(p, aes(y=value, x=Protein_legume_total, color=Total_feed_raw) ) + #ylab("Temperature difference (°C)")+ xlab("Height binned (m.a.s.l.)")+
  theme_bw()+theme(axis.line = element_line(colour = "black"),panel.background = element_blank(),strip.placement = "outside", plot.title=element_text(hjust=-0.2),strip.background = element_blank(),legend.key=element_rect(size=1,color="white"),legend.key.size = unit(1, "lines"),legend.position="top", panel.grid.minor = element_blank(),panel.grid.major = element_blank(),axis.text.x =element_text(angle = 45, hjust = 1),text = element_text(size=11),axis.title = element_text(size = 11))+
  geom_point(size=2)+
  # geom_boxplot(outlier.colour = "grey")+#+stat_boxplot(geom = "errorbar", width = 0.2)
  # geom_smooth(method='lm',formula=y~sqrt(x))+
  # scale_color_manual(values = tol10qualitative )+
  # scale_y_continuous(labels = scales::scientific)+
  facet_wrap(variable~Animal,switch="y", scales = "free",nrow=1)
...
Sum_digits <- setDT(p_conc)[,list(Feed_baseline=sum(Feed_baseline),Import=sum(Import),Legume_feed_CH=sum(Legume_feed_CH,na.rm=T),Reduction_in_conc_feed=sum(Reduction_in_conc_feed,na.rm = T),Feed_reduced=sum(Feed_reduced)),by=.(Scenario,Animal,Feed,Protein_legume_total,Total_feed_raw)]
Sum_digits$Import[Sum_digits$Import==0] <- NA
Sum_digits$Legume_feed_CH[Sum_digits$Legume_feed_CH==0] <- NA
...
ggR_ruminants <- ggplot(p, aes(y=Total_feed_raw,x=Protein_legume_total, color=(value) ))+ ylab("Raw feed produced (Mt DM/year)")+ xlab("Legume protein produced (Mt DM/year)")+
...
ggR_monogastric <- ggplot(p, aes(y=Total_feed_raw,x=Protein_legume_total, color=(value) )) + ylab("Raw feed produced (Mt DM/year)")+ xlab("Legume protein produced (Mt DM/year)")+
  theme_bw()+theme(axis.line = element_line(colour = "black"),panel.background = element_blank(),strip.placement = "outside", plot.title=element_text(hjust=-0.2),strip.background = element_blank(),legend.key=element_rect(size=1,color="white"),legend.key.width = unit(1.75, "lines"),legend.position="top", panel.grid.minor = element_blank(),panel.grid.major = element_blank(),axis.text.x =element_text(angle = 45, hjust = 1),text = element_text(size=11),axis.title = element_text(size = 11))+
  geom_point(size=3)+
  # geom_boxplot(outlier.colour = "grey")+#+stat_boxplot(geom = "errorbar", width = 0.2)
  # geom_smooth(method='lm',formula=y~sqrt(x))+
...
# ggsave("Sensitivity_production_on_r.pdf",  width = 180, height = 110, units = "mm", dpi = 300, first_row)
...
Sum_digits <- setDT(p_conc)[,list(Feed_baseline=sum(Feed_baseline),Import=sum(Import),Legume_feed_CH=sum(Legume_feed_CH,na.rm=T),Reduction_in_conc_feed=sum(Reduction_in_conc_feed,na.rm = T),Feed_reduced=sum(Feed_reduced)),by=.(Scenario,Feed,Protein_legume_total,Total_feed_raw)]
Sum_digits$Import[Sum_digits$Import==0] <- NA
Sum_digits$Legume_feed_CH[Sum_digits$Legume_feed_CH==0] <- NA
Sum_digits$ReducedTo_Proportion <- Sum_digits$Feed_reduced/ Sum_digits$Feed_baseline
...
total$Legume_feed_CH <- total_animal$Legume_feed_CH[2]
...
areas_average_yield[] <- lapply(areas_average_yield, function(x) gsub(" \\(\u00B10\\)","", x))
average_yield_raw_digits <- p[,lapply(.SD, mean), by=.(Import_CH,Scenario,Animal, Feed, Feed_item)]


Sum_digits <- setDT(p)[,list(Feed_baseline=sum(Feed_baseline),Reduction_in_raw_feed_area=sum(Reduction_in_raw_feed_area), Reduction_in_raw_feed=sum(Reduction_in_raw_feed),Feed_reduced=sum(Feed_reduced)), by=.(Scenario,Animal,Feed,Protein_legume_total,Total_feed_raw)]
Sum_digits$Protein_legume_total <- NULL
Sum_digits$Total_feed_raw <- NULL
Sum_digits$Reduction_in_raw_feed_area[Sum_digits$Reduction_in_raw_feed_area==0] <- NA
Sum_digits$ReducedTo_Proportion <- Sum_digits$Feed_reduced/ Sum_digits$Feed_baseline
...
Sum_digits <- setDT(p)[,list(Feed_baseline=sum(Feed_baseline),Reduction_in_raw_feed_area=sum(Reduction_in_raw_feed_area), Reduction_in_raw_feed=sum(Reduction_in_raw_feed),Feed_reduced=sum(Feed_reduced)), by=.(Scenario,Feed,Protein_legume_total,Total_feed_raw)]
Sum_digits$ReducedTo_Proportion <- Sum_digits$Feed_reduced/ Sum_digits$Feed_baseline
...
total <- Sum_digits[,lapply(.SD, function(x) paste0(format(round(mean(x),digits = 0), big.mark=",")," (\u00B1",format(round(sd(x,na.rm=T),digits = 0), big.mark=","),")")), by=.(Feed, Scenario)]
total$ReducedTo_Proportion <- dummy_digits_rot$ReducedTo_Proportion
total$Reduction_in_raw_feed_area <- total_animal$Reduction_in_raw_feed_area[1]
total[] <- lapply(total, function(x) gsub(" \\(\u00B10\\)","", x))
total$Animal <- "Total"
total$Feed_item <- "All"


table_reduced_raw_feed_total <- rbind.fill(areas_average_yield, total_animal, total )
table_reduced_raw_feed_total


p <- table_reduced_raw_feed_total
...
require(xtable)
# options(xtable.sanitize.text.function=identity)
print(xtable(p, type = "latex", digits=c(1)), file = "table_reduced_raw_feed_total.tex",include.rownames=F, hline.after=c(-1,1,nrow(p)-3,nrow(p)-1,nrow(p)))
...
ggplot(p, aes(y=value,x=as.factor(Protein_legume_total), color=Animal, shape=Animal )) + ylab("Reduction to")+ xlab("Legume protein produced in CH (t/year)")+
  theme_bw()+theme(axis.line = element_line(colour = "black"),panel.background = element_blank(),strip.placement = "outside", plot.title=element_text(hjust=-0.2),strip.background = element_blank(),legend.key=element_rect(size=1,color="white"),legend.key.size = unit(1, "lines"),legend.position="top", panel.grid.minor = element_blank(),panel.grid.major = element_blank(),axis.text.x =element_text(angle = 45, hjust = 1),text = element_text(size=11),axis.title = element_text(size = 11))+
  # geom_point(size=3)+
  geom_boxplot(outlier.colour = "grey")+#+stat_boxplot(geom = "errorbar", width = 0.2)
  # geom_smooth(method='lm',formula=y~sqrt(x))+
...
average_yield_raw_digits

InOutConc <- rbind.fill(average_yield_conc_digits, average_yield_raw_digits)
...
InOutConc$Legume_feed_CH[is.na(InOutConc$Legume_feed_CH)] <- 0
InOutConc$Saved_total <- InOutConc$Quantity-InOutConc$Feed_reduced
InOutConc$Legume_feed_CH[(InOutConc$Feed_item%in%c("Side Products")&InOutConc$Animal=="Monogastrics")] <- InOutConc$Legume_feed_CH[(InOutConc$Feed_item%in%c("Side Products")&InOutConc$Animal=="Monogastrics")]-10000 ## balance to avoid negativ numbers
InOutConc$Legume_feed_CH[(InOutConc$Feed_item%in%c("Conc others")&InOutConc$Animal=="Monogastrics")] <- 10000 ## balance to avoid negativ numbers


InOutConc$Feed_reduced <- InOutConc$Feed_reduced-InOutConc$Legume_feed_CH
InOutConc$Import <- abs(InOutConc$Import)
InOutConc$Saved <- InOutConc$Saved_total - InOutConc$Import
InOut_Scenario <- melt.data.table(InOutConc, id.vars = c("Animal","Feed","Feed_item"),measure.vars = c("Import","Feed_reduced","Legume_feed_CH","Saved"),variable.name = "Scenario")
InOut_Scenario <- subset(InOut_Scenario, Feed_item!="Add_Raw_feed_from_monogastric" )
...
InOut$cols <- "grey"
  ...
  InOut$cols[InOut$Scenario == "Saved_imports"] <- "#117733"
    ...
    ggProteinFluxes <- ggplot(p, aes(y = Protein, axis1 = Source, axis2= Scenario)) + ylab("Food protein produced (Mt DM/year)")+
      ...
      # scale_x_continuous(breaks = 1:2, labels = c("Sex", "Class")) + 
      scale_fill_manual( values=tol4qualitative[c(3,1,2)])+
      geom_text(stat  = 'stratum',label.strata =  TRUE)+
      theme_bw()+ theme(legend.position = "none")
